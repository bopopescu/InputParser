...
class Scheduler(object):
  ...
  def registered(self, driver, frameworkId, mainInfo):
    """
      ...
      main, and the mainInfo which is information about the main
      itself.
    """
  ...
  def reregistered(self, driver, mainInfo):
    """
      ...
      main.  This is only called when the scheduler has previously been
      registered.  mainInfo contains information about the newly elected
      main.
    """

  def disconnected(self, driver):
    """
      Invoked when the scheduler becomes disconnected from the main, e.g.
      the main fails and another is taking over.
    """
  ...
  def resourceOffers(self, driver, offers):
    """
      ...
      offer will only contain resources from a single subordinate.  Resources
  ...
  def offerRescinded(self, driver, offerId):
    """
      Invoked when an offer is no longer valid (e.g., the subordinate was lost or
  ...
  def inverseOfferRescinded(self, driver, offerId):
    """
      Invoked when an inverse offer is no longer valid (e.g., the subordinate was lost or
  ...
  def statusUpdate(self, driver, status):
    """
      Invoked when the status of a task has changed (e.g., a subordinate is
      ...
      however, that this is currently not true if the subordinate sending the
  ...
  def operationStatusUpdate(self, driver, status):
    """
      ...
      main, agent, or resource provider. These updates are only sent to
  ...
  def subordinateLost(self, driver, subordinateId):
    """
      Invoked when a subordinate has been determined unreachable (e.g., machine
      failure, network partition.) Most frameworks will need to reschedule
      any tasks launched on this subordinate on a new subordinate.

      NOTE: This callback is not reliably delivered. If a host or
      network failure causes messages between the main and the
...
class SchedulerDriver(object):
  ...
  def requestResources(self, requests):
    """
      ...
      subordinates.)  Any resources available are offered to the framework via
  ...
  def launchTasks(self, offerIds, tasks, filters=None):
    """
      ...
      offers must belong to the same subordinate. Invoking this function with
  ...
  def acceptOffers(self, offerIds, operations, filters=None):
    """
      ...
      belong to the same subordinate.
    """
  ...
  def reviveOffers(self):
    """
      ...
      those filtered subordinates.
    """

  def suppressOffers(self):
    """
      Inform Mesos main to stop sending offers to the framework. The
  ...
  def sendFrameworkMessage(self, executorId, subordinateId, data):
    """
  ...
  def reconcileTasks(self, tasks):
    """
      ...
      This causes the main to send back the latest task status for
      each task in 'statuses', if possible. Tasks that are no longer
      known will result in a TASK_LOST update. If statuses is empty,
      then the main will send the latest status for each task
      currently known.
    """
...
class Executor(object):
  ...
  def disconnected(self, driver):
    """
      Invoked when the executor becomes "disconnected" from the subordinate (e.g.,
      the subordinate is being restarted due to an upgrade).
    """
...
class OperatorDaemonDriver(object):
  """
    Operator HTTP API: Operations common to main daemon and agent daemon.
  """

  def getHealth(self):
    """
      This call retrieves the health status of main daemon or agent daemon.
    """
  ...
  def setLoggingLevel(self, level, duration):
    """
      Sets the logging verbosity level for a specified duration for main daemon or agent daemon. Mesos uses glog for
  ...
  def listFiles(self, path):
    """
      This call retrieves the file listing for a directory in main daemon or agent daemon.
    """
  ...
  def getFrameworks(self):
    """
      This call retrieves information about all the frameworks known to the main daemon or agent daemon.
    """

  def getExecutors(self):
    """
      Queries about all the executors known to the main daemon or agent daemon.
    """

  def getTasks(self):
    """
      Query about all the tasks known to the main daemon or agent daemon.
    """


class OperatorMainDriver(OperatorDaemonDriver):
  """
  ...
  def abort(self):
    """
      ...
      separated so that code can detect an aborted driver (i.e., via the
      return status of OperatorMainDriver.join), and instantiate and start
      another driver if desired (from within the same process, although this
      functionality is currently not supported for executors).
    """
  ...
  def getAgents(self):
    """
      This call retrieves information about all the agents known to the main.
    """
  ...
  def getMain(self):
    """
      This call retrieves the information on main.
    """
  ...
  def markAgentGone(self, agent_id):
    """
      ...
      (e.g., ephemeral instance from cloud provider). The main would shutdown the agent and send TASK_GONE_BY_OPERATOR
      ...
      operator's assertion was wrong and the agent was partitioned away from the main. The agent would be shutdown
      when it tries to re-register with the main when the partition heals. This call is idempotent.
    """

class OperatorMain(object):
  """
  ...
  # def heartbeat(self):
  #   """
  #     Periodically sent by the main to the subscriber according to 'Subscribed.heartbeat_interval_seconds'. If the
  ...
  def taskAdded(self, task_info):
    """
      Invoked whenever a task has been added to the main. This can happen either when a new task launch is processed
      by the main or when an agent re-registers with a failed over main.
    """

  def taskUpdated(self, task_info):
    """
      Invoked whenever the state of the task changes in the main. This can happen when a status update is received or
      generated by the main. Since status updates are retried by the agent, not all status updates received by the
      main result in the event being sent.
    """

  def frameworkAdded(self, framework_info):
    """
      Sent whenever a framework becomes known to the main. This can happen when a new framework registers with the
      main.
    """

  def frameworkUpdated(self, framework_info):
    """
      Sent whenever a framework re-registers with the main upon a disconnection (network error) or upon a main
      failover.
    """
  ...
  def frameworkRemoved(self, framework_info):
    """
      ...
      if it fails to re-register with the main within the failover timeout.
    """
  ...
  def agentAdded(self, agent_info):
    """
      ...
      reregistered after a main failover.
    """
  ...
  def agentRemoved(self, agent_id):
    """
      ...
      possible that an agent might become active once it has been removed, i.e. if the main has gc'ed its list of
...
class OperatorAgentDriver(OperatorDaemonDriver):
  ...
  def removeResourceProviderConfig(self, type, name):
    """
      ...
      the config is added back. The main and the agent will think the resource provider has disconnected, similar to
      agent disconnection.
      ...
      resource provider is disconnected, the main will rescind the offers related to that local resource provider,
