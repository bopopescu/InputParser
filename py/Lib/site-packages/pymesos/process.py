        ...
        try:
            ...
            if self._stream_id is None and self._parser.is_headers_complete():
                ...
                if code == TEMPORARY_REDIRECT:
                    ...
                    new_main = headers['LOCATION']
                    new_main = urlparse(new_main).netloc or new_main
                    logger.warning(
                        'Try to redirect to new main: %s', new_main
                    )
                    self._callback.change_main(new_main)
                    return False

                elif code == SERVICE_UNAVAILABLE:
                    logger.warning('Main is not available, retry.')
                    return False
...
class Process(object):
    ...
    def __init__(self, main=None, timeout=DAY):
        self._main = None
        self._started = False
        self._lock = RLock()
    ...
    def _run(self):
        try:
            ...
            while True:
                if not conn and self._main:
                    conn = Connection(self._main, self)
                ...
                if not conn.connected:
                    ...
                    if err:
                        ...
                        logger.warning(
                            ...
                            ' deadline: %s',
                            self._main, err, sleep_timeout, deadline)
