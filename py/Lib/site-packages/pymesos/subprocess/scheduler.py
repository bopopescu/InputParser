...
class ProcScheduler(Scheduler):
    ...
    def __init__(self):
        ...
        self.executor = None
        self.main = str(CONFIG.get('main', os.environ['MESOS_MASTER']))
        self.principal = str(
        ...
        self.driver = MesosSchedulerDriver(
            self, self.framework, self.main, principal=self.principal,
            secret=self.secret
        )
        self.procs_pending = {}
        self.procs_launched = {}
    ...
    def registered(self, driver, framework_id, main_info):
        with self._lock:
            logger.info('Framework registered with id=%s, main=%s' % (
                framework_id, main_info))
    ...
    def subordinateLost(self, driver, agent_id):
        agent_id = agent_id['value']
        with self._lock:
            for proc_id in self.agent_to_proc.pop(agent_id, []):
                self._call_finished(
