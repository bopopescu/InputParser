...
class MesosSchedulerDriver(Process, SchedulerDriver):
    ...
    def __init__(self, sched, framework, main_uri,
    ...
    def _get_version(self, main):
        if main is not None:
            conn = None
            host, port = main.split(':', 2)
            port = int(port)
            try:
    ...
    def change_main(self, main):
        self.version = self._get_version(main)
        super(MesosSchedulerDriver, self).change_main(main)
        self._close()
    ...
    def start(self):
        ...
        if uri.startswith('zk://') or uri.startswith('zoo://'):
            from .detector import MainDetector
            self.detector = MainDetector(uri[uri.index('://') + 3:], self)
            self.detector.start()
        else:
            if ':' not in uri:
                uri += ':5050'
            self.change_main(uri)
    ...
    def onNewMainDetectedMessage(self, data):
        main = None
        try:
        ...
        if main:
            self.change_main(main)

    def onNoMainDetectedMessage(self):
        self.change_main(None)
    ...
    def gen_request(self):
        ...
        request = ('POST /api/v1/scheduler HTTP/1.1\r\nHost: %s\r\n'
                   ...
                   'Connection: close\r\nContent-Length: %s\r\n\r\n%s') % (
                       self.main, _authorization, len(data), data
        )
    ...
    def on_subscribed(self, info):
        ...
        hostname, port = self.main.split(':', 2)
        port = int(port)
        main_info = dict(
            hostname=hostname,
            port=port,
        ...
        if self.version:
            main_info['version'] = self.version
        elif 'main_info' in info and 'version' in info['main_info']:
            main_info['version'] = info['main_info']['version']

        if reregistered:
            self.sched.reregistered(self, self._dict_cls(main_info))
        else:
            framework_id = dict(
    ...
    def on_failure(self, failure):
        ...
        if 'executor_id' not in failure:
            self.sched.subordinateLost(self, self._dict_cls(agent_id))
        else:
