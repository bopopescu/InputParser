...
notes:
    ...
    - At least one of C(main) or C(logging) params must be supplied.
    - When C(state=absent), boolean parameters are flipped,
      e.g. C(main=true) will disable the authoritative server.
    - When C(state=absent) and C(main=true), the stratum will be removed as well.
    - When C(state=absent) and C(main=false), the stratum will be configured
...
updates:
    ...
    returned: always
    type: list
    sample: ["no ntp logging", "ntp main 11"]
'''
import re
...
def get_current(module):
    ...
    match = re.search(r"^ntp main(?: (\d+))", output[0], re.M)
    if match:
        main = True
...
def main():
    ...
    desired = {'main': main, 'stratum': stratum, 'logging': logging}
    ...
    if state == 'absent':
        ...
        if desired['main'] and desired['main'] != current['main']:
            if desired['stratum']:
