...
class GalaxyInteractorApi(object):
    ...
    def __init__(self, **kwds):
        ...
        self.main_api_key = kwds["main_api_key"]
        self.api_key = self.__get_user_key(kwds.get("api_key"), kwds.get("main_api_key"), test_user=kwds.get("test_user"))
        if kwds.get('user_api_key_is_admin_key', False):
            self.main_api_key = self.api_key
    ...
    def __inject_api_key(self, data, key, admin, anon):
        ...
        if not anon:
            if not key:
                key = self.api_key if not admin else self.main_api_key
            params['key'] = key
        return params, data
