...
"""
    ...
    WORKER_DOCKER_ARGS = '--work_dir=/var/lib/mesos --main={ip}:5050 --attributes=preemptable:{preemptable} --no-hostname_lookup --no-systemd_enable_support'


    def _getCloudConfigUserData(self, role, mainPublicKey=None, keyPath=None, preemptable=False):
        if role == 'leader':
        ...
        templateArgs = dict(role=role,
                            ...
                            entrypoint=entryPoint,
                            sshKey=mainPublicKey,   # ignored if None
                            mesosArgs=mesosArgs)
